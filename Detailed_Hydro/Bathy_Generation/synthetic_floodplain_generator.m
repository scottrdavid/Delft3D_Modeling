...
do_output_xyz_for_delft3D=0; 
do_calc_downstream_elevation=1;%%% written to varible blinfo;
outputneg=1;

s=[4:1:10 20:10:100 200:100:400]';
% s=200;
% s=s.*(10^-5);
blinfo=ones(3,length(s))*9999;%%% downstream bed level informaiton
...
%%%Floodplain/River
    ...
    fpslope=s(i)*(10^-5);      %%% slope of the floodplain (-)
...
%%%Trench
tdepth=1;
twidth=105;%%% need to add 15 to whatever you actually want
dist=500;

%%% Outputfiles
modelorigin=100;%%% delft hates starting at 0,0 so using 100,100 in my runs
xyzfilename=(['S',num2str(s(i)),'Tw',num2str(twidth-15),'.xyz'])%'Tw',num2str(twidth-15),
...
if do_calc_downstream_elevation==1
    blinfo(1,i)=s(i);
    blinfo(2,i)=nanmean(fpmat(:,234));
    blinfo(3,i)=abs(nanmean(fpmat(:,234)))+0.3;
else
end
